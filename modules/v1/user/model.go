...
func (item *User) Create(db *gorm.DB) (uint, error) {
	...
	if err != nil {
		myzap.ZAPLOG.Error("添加失败", zap.String("(item *User) Create()", err.Error()))
		return item.ID, err
	}
	return item.ID, nil
}
...
func (item *User) Update(db *gorm.DB, scopes ...func(db *gorm.DB) *gorm.DB) error {
	...
	if err != nil {
		myzap.ZAPLOG.Error("更新失败", zap.String("(item *User) Update() ", err.Error()))
		return err
	}
	return nil
}
...
func (item *User) Delete(db *gorm.DB, scopes ...func(db *gorm.DB) *gorm.DB) error {
	...
	if err != nil {
		myzap.ZAPLOG.Error("删除失败", zap.String("(item *User) Delete()", err.Error()))
		return err
	}
	return nil
}
